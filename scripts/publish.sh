...
if git diff --cached --quiet; then
  notify_discord "ℹ️ Alberta Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: AB daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
git remote get-url origin && git push origin main || true

export PATH="/Users/clawii/.npm-global/bin:$PATH"
wrangler pages deploy . --project-name ab-payhub --branch main 2>&1 | tail -3 || true
...
notify_discord "✅ Alberta Pay Hub updated [$TODAY]
