#!/usr/bin/env python3
"""
ab-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Alberta edition.
...
Run: python3 ~/ab-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/ab-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/ab-pay-hub/scripts/.greenhouse.lock")
...
SALARY_PATTERNS = [
    r'\CA$\s*([\\d,]+)\s*[-–—]\s*\CA$\s*([\\d,]+)',
    r'([\\d,]+)\s*[-–—]\s*([\\d,]+)\s*(?:CAD|cad)',
    r'salary[:\s]+\CA$?([\\d,]+)[kK]?\s*[-–—]\s*\CA$?([\\d,]+)[kK]?',
    r'compensation[:\s]+\CA$?([\\d,]+)[kK]?\s*[-–—]\s*\CA$?([\\d,]+)[kK]?',
    r'pay range[:\s]+\CA$?([\\d,]+)[kK]?\s*[-–—]\s*\CA$?([\\d,]+)[kK]?',
...
def is_ab_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_ab_job(title, location, content_text):
            continue
...
def main():
    ...
    log(f"=== Done. {new_count} new AB jobs written to {OUTPUT_FILE} ===")
