#!/usr/bin/env python3
"""
ab-pay-hub/scripts/_common.py
Shared utilities for all Alberta Pay Hub search scripts.
Alberta Employment Standards Code — no mandatory salary posting disclosure (yet). Many Calgary/Edmonton employers voluntarily include pay ranges following BC/Ontario precedent.
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"ab-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/ab-pay-hub/data/jobs.json")
CURRENCY    = "CAD"
REGION      = "AB"
...
REGION_TERMS = ['alberta', 'calgary', 'edmonton', 'red deer', 'lethbridge', 'fort mcmurray', 'medicine hat', 'grande prairie', 'airdrie', 'st. albert', 'leduc', 'spruce grove', 'okotoks', 'cochrane, ab', 'banff', 'canmore', ', ab', 'ab,', 'ab canada', 'oil sands', 'yeg', 'yyc']

_NON_REGION_LOC_TERMS = ['ontario', 'toronto', 'british columbia', 'vancouver', 'prince edward island', 'nova scotia', 'washington, dc', 'washington dc', 'denver, co', 'boulder, co', 'seattle, wa', 'new york city', 'new york, ny', 'san francisco, ca', 'chicago, il', 'boston, ma', 'newark, nj', 'baltimore, md', 'minneapolis, mn', 'burlington, vt']
...
EXTRACT_PROMPT = """\
Extract ONE Alberta job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Alberta job with explicit CAD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Alberta location","source_url":"{url}","posted":"YYYY-MM-DD"}
