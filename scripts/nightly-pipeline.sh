#!/usr/bin/env bash
# ab-pay-hub/scripts/nightly-pipeline.sh
# Nightly pipeline for Alberta Pay Hub.
...
SCRIPTS_DIR="$HOME/ab-pay-hub/scripts"
...
log "=== AB nightly pipeline started ==="
...
# === Phase 3 discovery — hub: ab (added 2026-05-27) ===
log "--- Phase 3 discovery: hub_search_jobs ---"
python3 "$HOME/shared-scripts/hub_search_jobs.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
log "hub_search_jobs done (exit $?)"
log "--- Phase 3 discovery: hub_deep_search ---"
python3 "$HOME/shared-scripts/hub_deep_search.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
log "hub_deep_search done (exit $?)"
log "--- Phase 3 discovery: hub_search_kpmg ---"
python3 "$HOME/shared-scripts/hub_search_kpmg.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
log "hub_search_kpmg done (exit $?)"
log "--- Phase 3 discovery: hub_search_sap ---"
python3 "$HOME/shared-scripts/hub_search_sap.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
log "hub_search_sap done (exit $?)"
log "--- Phase 3 discovery: hub_search_scout ---"
python3 "$HOME/shared-scripts/hub_search_scout.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
log "hub_search_scout done (exit $?)"

# === Phase 5 classify — hub: ab (added 2026-05-28) ===
log "--- Phase 5 classify_step ---"
python3 "$HOME/shared-scripts/region_classifier/classify_step.py" \
    --hub "ab" --no-llm >> "$LOG_FILE" 2>&1 || true
log "classify_step done (exit $?)"
# === Phase 5.5 publish gate — hub: ab (added 2026-05-28) ===
...
GATE_RAW="$HOME/.openclaw/shared/ab-jobs-raw-$(date +%Y-%m-%d).txt"
if [[ -f "$GATE_RAW" ]]; then
...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only --hub "ab" >> "$LOG_FILE" 2>&1
log "healthcheck done (exit $?)"

log "--- archive-jobs ---"
python3 "$HOME/shared-scripts/hub_archive_jobs.py" --hub "ab" --limit 100 >> "$LOG_FILE" 2>&1
log "archive done (exit $?)"
log "--- skill-extract ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_skill_extract.py" --hub "ab" --limit 200 >> "$LOG_FILE" 2>&1
log "skill-extract done (exit $?)"
log "--- classify-new-jobs ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_classify_new_jobs.py" --hub "ab" >> "$LOG_FILE" 2>&1 || true
...
log "=== AB nightly pipeline complete ==="
...
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only \
  --hub ab --pipeline-exit-code "$PIPELINE_RC" || true
exit "$PIPELINE_RC"
