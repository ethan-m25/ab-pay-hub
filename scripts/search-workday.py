#!/usr/bin/env python3
"""
ab-pay-hub/scripts/search-workday.py
Workday API scraper — Alberta edition.

Alberta voluntary salary disclosure

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Alberta employers
  2. CXS JSON API   — paginate all jobs per employer, filter Alberta (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/ab-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/ab-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/ab-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Alberta"
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Alberta" Canada job salary 2026',
    'site:myworkdayjobs.com "Calgary" OR "Edmonton" salary "$" CAD 2026',
    'site:myworkdayjobs.com "Alberta" Canada engineer OR analyst OR manager salary 2026',
    'site:myworkdayjobs.com "Suncor" OR "Cenovus" OR "CNRL" Alberta salary',
    'site:myworkdayjobs.com Alberta energy OR oil OR gas salary range 2026',
    'site:myworkdayjobs.com Calgary OR Edmonton tech OR finance salary 2026',
    'site:myworkdaysite.com "Alberta" OR "Calgary" Canada salary 2026',
]
...
def is_alberta(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Alberta.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_alberta(p.get("locationsText", ""), p.get("externalPath", "")):
                    ab_jobs.append(p)
            offset += limit
        ...
        log(f"  Alberta jobs: {len(ab_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(ab_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_alberta(location, external_path):
            continue
